...
class DoublePendulum():
    ...
    def __init__(self):
        ...
        """
        self.m1 = self.m2 = self.l1 = self.l2 = self.g = 1
    ...
    def H(self, x):
        ...
        return ( (p1**2 + 2 * p2**2 - 2 * p1 * p2 * np.cos(q1 - q2) ) / (2 * (1 + np.sin(q1 - q2)**2)) ) - 2 * np.cos(q1) - np.cos(q2)
    def dH(self, x):
        """
        ...
        dq1 = 2 * np.sin(q1) + h1 - h2 * np.sin(2 * (q1 - q2))
        dq2 = np.sin(q2) - h1 + h2 * np.sin(2 * (q1 - q2))
        dp1 = (p1 - p2 * np.cos(q1 - q2)) / (1 + np.sin(q1 - q2)**2)
        dp2 = (2 * p2 - p1 * np.cos(q1 - q2)) / (1 + np.sin(q1 - q2)**2)
